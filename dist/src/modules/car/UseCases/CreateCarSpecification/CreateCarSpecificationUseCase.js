...
let CreateCarSpecificationUseCase = class CreateCarSpecificationUseCase {
    constructor(carsRespository, specificationsRepository) {
        this.carsRespository = carsRespository;
        this.specificationsRepository = specificationsRepository;
    }
    execute({ car_id, specifications_id }) {
        return __awaiter(this, void 0, void 0, function* () {
            const carExists = yield this.carsRespository.findById(car_id);
            if (!carExists) {
                throw new AppError_1.AppError('Car does not exists');
            }
            const specificarion = yield this.specificationsRepository.findByIds(specifications_id);
            carExists.specifications = specificarion;
            yield this.carsRespository.create(carExists);
            return carExists;
        });
    }
};
